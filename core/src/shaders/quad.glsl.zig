const sg = @import("sokol").gfx;
const std = @import("std");
const m = @import("../math.zig");
//
//    #version:1# (machine generated, don't edit!)
//
//    Generated by sokol-shdc (https://github.com/floooh/sokol-tools)
//
//    Cmdline:
//        sokol-shdc --input src/shaders/quad.glsl --output src/shaders/quad.glsl.zig -l glsl410:metal_macos:hlsl5:glsl300es:wgsl -f sokol_zig --reflection
//
//    Overview:
//    =========
//    Shader program: 'quad':
//        Get shader desc: shd.quadShaderDesc(sg.queryBackend());
//        Vertex Shader: vs
//        Fragment Shader: fs
//        Attributes:
//            ATTR_quad_position => 0
//            ATTR_quad_color0 => 1
//            ATTR_quad_instance_position => 2
//    Bindings:
//        Uniform block 'vs_params':
//            Zig struct: VsParams
//            Bind slot: UB_vs_params => 0
//
pub const ATTR_quad_position = 0;
pub const ATTR_quad_color0 = 1;
pub const ATTR_quad_instance_position = 2;
pub const UB_vs_params = 0;
pub const VsParams = extern struct {
    zoom: m.Mat4 align(16),
    window_scale: m.Mat4 align(1),
    untransform: m.Mat4 align(1),
};
//
//    #version 410
//
//    uniform vec4 vs_params[12];
//    layout(location = 0) in vec2 position;
//    layout(location = 2) in vec2 instance_position;
//    layout(location = 0) out vec2 uv;
//    layout(location = 1) out vec4 color;
//    layout(location = 1) in vec4 color0;
//
//    void main()
//    {
//        vec2 _53 = vec2(position.x, position.y);
//        gl_Position = ((mat4(vs_params[8], vs_params[9], vs_params[10], vs_params[11]) * mat4(vs_params[0], vs_params[1], vs_params[2], vs_params[3])) * mat4(vs_params[4], vs_params[5], vs_params[6], vs_params[7])) * (vec4(instance_position.x, instance_position.y, 1.0, 1.0) + vec4(_53 * (exp(log(vs_params[0].x) * 0.00999999977648258209228515625) * 0.0033333334140479564666748046875), 0.0, 0.0));
//        uv = _53;
//        color = color0;
//    }
//
//
const vs_source_glsl410 = [729]u8 {
    ,
};
//
//    #version 410
//
//    layout(location = 0) in vec2 uv;
//    layout(location = 0) out vec4 frag_color;
//    layout(location = 1) in vec4 color;
//
//    void main()
//    {
//        if (distance(uv, vec2(0.0)) > 0.5)
//        {
//            discard;
//        }
//        frag_color = color;
//    }
//
//
const fs_source_glsl410 = [236]u8 {
    ,
};
//
//    #version 300 es
//
//    uniform vec4 vs_params[12];
//    layout(location = 0) in vec2 position;
//    layout(location = 2) in vec2 instance_position;
//    out vec2 uv;
//    out vec4 color;
//    layout(location = 1) in vec4 color0;
//
//    void main()
//    {
//        vec2 _53 = vec2(position.x, position.y);
//        gl_Position = ((mat4(vs_params[8], vs_params[9], vs_params[10], vs_params[11]) * mat4(vs_params[0], vs_params[1], vs_params[2], vs_params[3])) * mat4(vs_params[4], vs_params[5], vs_params[6], vs_params[7])) * (vec4(instance_position.x, instance_position.y, 1.0, 1.0) + vec4(_53 * (exp(log(vs_params[0].x) * 0.00999999977648258209228515625) * 0.0033333334140479564666748046875), 0.0, 0.0));
//        uv = _53;
//        color = color0;
//    }
//
//
const vs_source_glsl300es = [690]u8 {
    ,
};
//
//    #version 300 es
//    precision mediump float;
//    precision highp int;
//
//    in highp vec2 uv;
//    layout(location = 0) out highp vec4 frag_color;
//    in highp vec4 color;
//
//    void main()
//    {
//        if (distance(uv, vec2(0.0)) > 0.5)
//        {
//            discard;
//        }
//        frag_color = color;
//    }
//
//
const fs_source_glsl300es = [261]u8 {
    ,
};
//
//    cbuffer vs_params : register(b0)
//    {
//        row_major float4x4 _13_zoom : packoffset(c0);
//        row_major float4x4 _13_window_scale : packoffset(c4);
//        row_major float4x4 _13_untransform : packoffset(c8);
//    };
//
//
//    static float4 gl_Position;
//    static float2 position;
//    static float2 instance_position;
//    static float2 uv;
//    static float4 color;
//    static float4 color0;
//
//    struct SPIRV_Cross_Input
//    {
//        float2 position : TEXCOORD0;
//        float4 color0 : TEXCOORD1;
//        float2 instance_position : TEXCOORD2;
//    };
//
//    struct SPIRV_Cross_Output
//    {
//        float2 uv : TEXCOORD0;
//        float4 color : TEXCOORD1;
//        float4 gl_Position : SV_Position;
//    };
//
//    void vert_main()
//    {
//        float2 _53 = float2(position.x, position.y);
//        gl_Position = mul(float4(instance_position.x, instance_position.y, 1.0f, 1.0f) + float4(_53 * (exp(log(_13_zoom[0].x) * 0.00999999977648258209228515625f) * 0.0033333334140479564666748046875f), 0.0f, 0.0f), mul(_13_window_scale, mul(_13_zoom, _13_untransform)));
//        uv = _53;
//        color = color0;
//    }
//
//    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
//    {
//        position = stage_input.position;
//        instance_position = stage_input.instance_position;
//        color0 = stage_input.color0;
//        vert_main();
//        SPIRV_Cross_Output stage_output;
//        stage_output.gl_Position = gl_Position;
//        stage_output.uv = uv;
//        stage_output.color = color;
//        return stage_output;
//    }
//
const vs_source_hlsl5 = [1352]u8 {
    ,
};
//
//    static float2 uv;
//    static float4 frag_color;
//    static float4 color;
//
//    struct SPIRV_Cross_Input
//    {
//        float2 uv : TEXCOORD0;
//        float4 color : TEXCOORD1;
//    };
//
//    struct SPIRV_Cross_Output
//    {
//        float4 frag_color : SV_Target0;
//    };
//
//    void frag_main()
//    {
//        if (distance(uv, 0.0f.xx) > 0.5f)
//        {
//            discard;
//        }
//        frag_color = color;
//    }
//
//    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
//    {
//        uv = stage_input.uv;
//        color = stage_input.color;
//        frag_main();
//        SPIRV_Cross_Output stage_output;
//        stage_output.frag_color = frag_color;
//        return stage_output;
//    }
//
const fs_source_hlsl5 = [572]u8 {
    ,
};
//
//    #include <metal_stdlib>
//    #include <simd/simd.h>
//
//    using namespace metal;
//
//    struct vs_params
//    {
//        float4x4 zoom;
//        float4x4 window_scale;
//        float4x4 untransform;
//    };
//
//    struct main0_out
//    {
//        float2 uv [[user(locn0)]];
//        float4 color [[user(locn1)]];
//        float4 gl_Position [[position]];
//    };
//
//    struct main0_in
//    {
//        float2 position [[attribute(0)]];
//        float4 color0 [[attribute(1)]];
//        float2 instance_position [[attribute(2)]];
//    };
//
//    vertex main0_out main0(main0_in in [[stage_in]], constant vs_params& _13 [[buffer(0)]])
//    {
//        main0_out out = {};
//        float2 _53 = float2(in.position.x, in.position.y);
//        out.gl_Position = ((_13.untransform * _13.zoom) * _13.window_scale) * (float4(in.instance_position.x, in.instance_position.y, 1.0, 1.0) + float4(_53 * (exp(log(_13.zoom[0].x) * 0.00999999977648258209228515625) * 0.0033333334140479564666748046875), 0.0, 0.0));
//        out.uv = _53;
//        out.color = in.color0;
//        return out;
//    }
//
//
const vs_source_metal_macos = [933]u8 {
    ,
};
//
//    #include <metal_stdlib>
//    #include <simd/simd.h>
//
//    using namespace metal;
//
//    struct main0_out
//    {
//        float4 frag_color [[color(0)]];
//    };
//
//    struct main0_in
//    {
//        float2 uv [[user(locn0)]];
//        float4 color [[user(locn1)]];
//    };
//
//    fragment main0_out main0(main0_in in [[stage_in]])
//    {
//        main0_out out = {};
//        if (distance(in.uv, float2(0.0)) > 0.5)
//        {
//            discard_fragment();
//        }
//        out.frag_color = in.color;
//        return out;
//    }
//
//
const fs_source_metal_macos = [430]u8 {
    ,
};
//
//    diagnostic(off, derivative_uniformity);
//
//    struct vs_params {
//      /_ @offset(0) _/
//      zoom : mat4x4f,
//      /_ @offset(64) _/
//      window_scale : mat4x4f,
//      /_ @offset(128) _/
//      untransform : mat4x4f,
//    }
//
//    @group(0) @binding(0) var<uniform> x_13 : vs_params;
//
//    var<private> position_1 : vec2f;
//
//    var<private> instance_position : vec2f;
//
//    var<private> uv : vec2f;
//
//    var<private> color : vec4f;
//
//    var<private> color0 : vec4f;
//
//    var<private> gl_Position : vec4f;
//
//    fn main_1() {
//      var t : mat4x4f;
//      var k : f32;
//      var size : f32;
//      var qp : vec2f;
//      var xy : vec4f;
//      let x_18 : mat4x4f = x_13.untransform;
//      let x_21 : mat4x4f = x_13.zoom;
//      let x_25 : mat4x4f = x_13.window_scale;
//      t = ((x_18 * x_21) * x_25);
//      let x_33 : f32 = x_13.zoom[0i].x;
//      k = x_33;
//      let x_35 : f32 = k;
//      size = (exp((log(x_35) * 0.00999999977648258209f)) / 300.0f);
//      let x_49 : f32 = position_1.x;
//      let x_52 : f32 = position_1.y;
//      qp = vec2f(x_49, x_52);
//      let x_58 : f32 = instance_position.x;
//      let x_60 : f32 = instance_position.y;
//      let x_63 : vec2f = qp;
//      let x_64 : f32 = size;
//      let x_65 : vec2f = (x_63 * x_64);
//      xy = (vec4f(x_58, x_60, 1.0f, 1.0f) + vec4f(x_65.x, x_65.y, 0.0f, 0.0f));
//      let x_75 : mat4x4f = t;
//      let x_76 : vec4f = xy;
//      gl_Position = (x_75 * x_76);
//      let x_82 : vec2f = qp;
//      uv = x_82;
//      let x_86 : vec4f = color0;
//      color = x_86;
//      return;
//    }
//
//    struct main_out {
//      @builtin(position)
//      gl_Position : vec4f,
//      @location(0)
//      uv_1 : vec2f,
//      @location(1)
//      color_1 : vec4f,
//    }
//
//    @vertex
//    fn main(@location(0) position_1_param : vec2f, @location(2) instance_position_param : vec2f, @location(1) color0_param : vec4f) -> main_out {
//      position_1 = position_1_param;
//      instance_position = instance_position_param;
//      color0 = color0_param;
//      main_1();
//      return main_out(gl_Position, uv, color);
//    }
//
//
const vs_source_wgsl = [1780]u8 {
    ,
};
//
//    diagnostic(off, derivative_uniformity);
//
//    var<private> uv : vec2f;
//
//    var<private> frag_color : vec4f;
//
//    var<private> color : vec4f;
//
//    fn main_1() {
//      var center : vec2f;
//      var dist : f32;
//      var radius : f32;
//      center = vec2f(0.0f, 0.0f);
//      let x_16 : vec2f = uv;
//      let x_17 : vec2f = center;
//      dist = distance(x_16, x_17);
//      radius = 0.5f;
//      let x_21 : f32 = dist;
//      let x_22 : f32 = radius;
//      if ((x_21 > x_22)) {
//        discard;
//      }
//      let x_33 : vec4f = color;
//      frag_color = x_33;
//      return;
//    }
//
//    struct main_out {
//      @location(0)
//      frag_color_1 : vec4f,
//    }
//
//    @fragment
//    fn main(@location(0) uv_param : vec2f, @location(1) color_param : vec4f) -> main_out {
//      uv = uv_param;
//      color = color_param;
//      main_1();
//      return main_out(frag_color);
//    }
//
//
const fs_source_wgsl = [734]u8 {
    ,
};
pub fn quadShaderDesc(backend: sg.Backend) sg.ShaderDesc {
    var desc: sg.ShaderDesc = .{};
    desc.label = "quad_shader";
    switch (backend) {
        .GLCORE => {
            desc.vertex_func.source = &vs_source_glsl410;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = &fs_source_glsl410;
            desc.fragment_func.entry = "main";
            desc.attrs[0].glsl_name = "position";
            desc.attrs[1].glsl_name = "color0";
            desc.attrs[2].glsl_name = "instance_position";
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 192;
            desc.uniform_blocks[0].glsl_uniforms[0].type = .FLOAT4;
            desc.uniform_blocks[0].glsl_uniforms[0].array_count = 12;
            desc.uniform_blocks[0].glsl_uniforms[0].glsl_name = "vs_params";
        },
        .GLES3 => {
            desc.vertex_func.source = &vs_source_glsl300es;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = &fs_source_glsl300es;
            desc.fragment_func.entry = "main";
            desc.attrs[0].glsl_name = "position";
            desc.attrs[1].glsl_name = "color0";
            desc.attrs[2].glsl_name = "instance_position";
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 192;
            desc.uniform_blocks[0].glsl_uniforms[0].type = .FLOAT4;
            desc.uniform_blocks[0].glsl_uniforms[0].array_count = 12;
            desc.uniform_blocks[0].glsl_uniforms[0].glsl_name = "vs_params";
        },
        .D3D11 => {
            desc.vertex_func.source = &vs_source_hlsl5;
            desc.vertex_func.d3d11_target = "vs_5_0";
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = &fs_source_hlsl5;
            desc.fragment_func.d3d11_target = "ps_5_0";
            desc.fragment_func.entry = "main";
            desc.attrs[0].hlsl_sem_name = "TEXCOORD";
            desc.attrs[0].hlsl_sem_index = 0;
            desc.attrs[1].hlsl_sem_name = "TEXCOORD";
            desc.attrs[1].hlsl_sem_index = 1;
            desc.attrs[2].hlsl_sem_name = "TEXCOORD";
            desc.attrs[2].hlsl_sem_index = 2;
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 192;
            desc.uniform_blocks[0].hlsl_register_b_n = 0;
        },
        .METAL_MACOS => {
            desc.vertex_func.source = &vs_source_metal_macos;
            desc.vertex_func.entry = "main0";
            desc.fragment_func.source = &fs_source_metal_macos;
            desc.fragment_func.entry = "main0";
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 192;
            desc.uniform_blocks[0].msl_buffer_n = 0;
        },
        .WGPU => {
            desc.vertex_func.source = &vs_source_wgsl;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = &fs_source_wgsl;
            desc.fragment_func.entry = "main";
            desc.uniform_blocks[0].stage = .VERTEX;
            desc.uniform_blocks[0].layout = .STD140;
            desc.uniform_blocks[0].size = 192;
            desc.uniform_blocks[0].wgsl_group0_binding_n = 0;
        },
        else => {},
    }
    return desc;
}
pub fn quadAttrSlot(attr_name: []const u8) ?usize {
    if (std.mem.eql(u8, attr_name, "position")) {
        return 0;
    }
    if (std.mem.eql(u8, attr_name, "color0")) {
        return 1;
    }
    if (std.mem.eql(u8, attr_name, "instance_position")) {
        return 2;
    }
    return null;
}
pub fn quadImageSlot(img_name: []const u8) ?usize {
    _ = img_name;
    return null;
}
pub fn quadSamplerSlot(smp_name: []const u8) ?usize {
    _ = smp_name;
    return null;
}
pub fn quadUniformblockSlot(ub_name: []const u8) ?usize {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
        return 0;
    }
    return null;
}
pub fn quadUniformblockSize(ub_name: []const u8) ?usize {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
        return @sizeOf(VsParams);
    }
    return null;
}
pub fn quadUniformOffset(ub_name: []const u8, u_name: []const u8) ?usize {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
        if (std.mem.eql(u8, u_name, "zoom")) {
            return 0;
        }
        if (std.mem.eql(u8, u_name, "window_scale")) {
            return 64;
        }
        if (std.mem.eql(u8, u_name, "untransform")) {
            return 128;
        }
    }
    return null;
}
pub fn quadUniformDesc(ub_name: []const u8, u_name: []const u8) ?sg.GlslShaderUniform {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
        if (std.mem.eql(u8, u_name, "zoom")) {
            var desc: sg.GlslShaderUniform = .{};
            desc.type = .MAT4;
            desc.array_count = 0;
            desc.glsl_name = "zoom";
            return desc;
        }
        if (std.mem.eql(u8, u_name, "window_scale")) {
            var desc: sg.GlslShaderUniform = .{};
            desc.type = .MAT4;
            desc.array_count = 0;
            desc.glsl_name = "window_scale";
            return desc;
        }
        if (std.mem.eql(u8, u_name, "untransform")) {
            var desc: sg.GlslShaderUniform = .{};
            desc.type = .MAT4;
            desc.array_count = 0;
            desc.glsl_name = "untransform";
            return desc;
        }
    }
    return null;
}
pub fn quadStoragebufferSlot(sbuf_name: []const u8) ?usize {
    _ = sbuf_name;
    return null;
}
